import { STANDX_I18N } from "./standxI18n";
...
export function initStandXGame() {
  ...
  function storedLang(){try{const v=localStorage.getItem("standx_lang");return LANGS.includes(v)?v:"pt";}catch(e){return"pt";}}
  let currentLang=storedLang();
  function localePack(){return STANDX_I18N[currentLang]||STANDX_I18N.pt;}
  function ui(){return localePack().ui||STANDX_I18N.pt.ui;}
  function tx(s){const p=localePack();return p.messages&&p.messages[s]?p.messages[s]:s;}
  function rankTx(s){const p=localePack();return p.ranks&&p.ranks[s]?p.ranks[s]:s;}
  function cloneData(v){
    if(Array.isArray(v))return v.map(cloneData);
    if(v&&typeof v==="object"){const o={};Object.keys(v).forEach(k=>{o[k]=cloneData(v[k]);});return o;}
    return v;
  }
  function mergeData(base,overlay){
    ...
    function merge(t,o){
      ...
      if(Array.isArray(t)&&Array.isArray(o)){o.forEach((v,i)=>{t[i]=merge(t[i],v);});return t;}
      if(t&&typeof t==="object"&&!Array.isArray(t)&&o&&typeof o==="object"&&!Array.isArray(o)){Object.keys(o).forEach(k=>{t[k]=merge(t[k],o[k]);});return t;}
      return cloneData(o);
    }
    return merge(out,overlay);
  }
  function setText(sel,val){const el=typeof sel==="string"?document.querySelector(sel):sel;if(el&&val!==undefined)el.textContent=val;}
  function setHTML(sel,val){const el=typeof sel==="string"?document.querySelector(sel):sel;if(el&&val!==undefined)el.innerHTML=val;}
  ...
  // ========== AUTO-SAVE ==========
  function autoSave(){
    try{localStorage.setItem("standx_save",JSON.stringify({lang:currentLang,flow:currentFlow,ep,rnk,zi,wx:camX,trg,gs:gs==="paused"?prevGS:gs}));}catch(e){}
  }
  function autoLoad(){
    try{
      const d=JSON.parse(localStorage.getItem("standx_save"));
      if(d&&d.ep!==undefined){
        const saveFlow=d.flow||"seed",saveZi=Number.isInteger(d.zi)?d.zi:0;
        if(saveFlow!==currentFlow||saveZi<0||saveZi>=ZONES.length)return null;
        ep=Math.min(Number(d.ep)||0,epMax);rnk=rankTx(d.rnk||rnk);zi=saveZi;camX=Number(d.wx)||0;wx=camX;trg=d.trg&&typeof d.trg==="object"?d.trg:{};
        document.getElementById("hep").textContent="EP: "+ep;
        document.getElementById("efl").style.width=(ep/epMax*100)+"%";
        document.getElementById("hrnk").textContent=rnk;
        document.getElementById("hzone").textContent=ZONES[zi].name;
        return d.gs==="playing"?"playing":null;
      }
    }catch(e){}return null;
  }
  ...
  function openHubDetail(rank){
    ...
    const hd=ui().hubDetail;
    activeHubDetailRank=rank;
    document.getElementById("hub").classList.remove("on");
    const el=document.getElementById("hubDetail");
    el.innerHTML="";
  
    // Build detail view
    let html=`<button class="hd-back" onclick="closeHubDetail()">${hd.back}</button>`;
    ...
    // Description
    html+=`<div class="hd-section" style="border-color:${d.color}22"><div class="hd-stitle" style="color:${d.color}"><span class="ico">&#128196;</span> ${hd.about}</div>`;
    ...
    // Milestones
    html+=`<div class="hd-section"><div class="hd-stitle" style="color:${d.color}"><span class="ico">&#127942;</span> ${hd.milestones}</div><ul class="hd-list">`;
    d.milestones.forEach(m=>{html+=`<li><span style="color:${d.color}">${m.ep.toLocaleString()} EP</span> — ${m.label}</li>`;});
    html+=`</ul></div>`;
  
    // Requirements
    html+=`<div class="hd-section"><div class="hd-stitle" style="color:${d.color}"><span class="ico">&#128221;</span> ${hd.requirements}</div><ul class="hd-list">`;
    d.requirements.forEach(r=>{html+=`<li>${r}</li>`;});
    html+=`</ul></div>`;
  
    // Activities
    html+=`<div class="hd-section"><div class="hd-stitle" style="color:${d.color}"><span class="ico">&#9889;</span> ${hd.activities}</div>`;
    ...
    // Squads
    html+=`<div class="hd-section"><div class="hd-stitle" style="color:${d.color}"><span class="ico">&#128101;</span> ${hd.squads}</div><div class="hd-squads">`;
    d.squads.forEach(s=>{
    ...
    // NPCs
    html+=`<div class="hd-section"><div class="hd-stitle" style="color:${d.color}"><span class="ico">&#128100;</span> ${hd.npcs}</div><div class="hd-npc-row" id="hdNpcs">`;
    d.npcs.forEach(id=>{
      const n=NPCS[id];
    ...
    // Benefits
    html+=`<div class="hd-section"><div class="hd-stitle" style="color:${d.color}"><span class="ico">&#11088;</span> ${hd.benefits}</div><ul class="hd-list">`;
    ...
    html+=`<div class="hd-stitle" style="color:${d.color}"><span class="ico">&#128161;</span> ${hd.tips}</div><ul class="hd-list">`;
    d.tips.forEach(t=>{html+=`<li>${t}</li>`;});
    html+=`</ul></div>`;
  
    // Play Flow Button
    html+=`<button class="hub-tbtn" style="margin-top:10px;width:100%;font-size:14px;padding:16px;background:rgba(${d.color==='#00e832'?'0,232,50':d.color==='#00aaff'?'0,170,255':'153,69,255'},.15);border-color:${d.color};color:${d.color}" onclick="selectFlow('${rank}');document.getElementById('tbtn').click()">${hd.play} ${d.rank} &#9654;</button>`;
  ...
  function mkPortrait(npcId,sz){
    ...
    (function anim(){if(started&&!c.isConnected)return;started=true;const cx=c.getContext("2d");cx.clearRect(0,0,sz,sz);drawChar(cx,npcId,sz,f++);requestAnimationFrame(anim);})();
    return c;
  }
  ...
  let QTE_CFG={
    trade:{
      title:"QTE -- EXECUCAO DE TRADE",sub:"PRESSIONE NO TIMING CERTO",action:"TRADE",
  ...
  const SPROUT_QTE={leadership:{title:"QTE -- DECISAO DE LIDERANCA",sub:"PRESSIONE NO TIMING CERTO",action:"LIDERAR",
  ...
  const FLOWER_QTE={innovation:{title:"QTE -- DECISAO DE INOVACAO",sub:"PRESSIONE NO TIMING CERTO",action:"INOVAR",
  ...
  const BASE_HUB_DATA=cloneData(HUB_DATA),BASE_NPCS=cloneData(NPCS);
  const BASE_FLOW_DATA={
    seed:{zones:SEED_ZONES,story:SEED_STORY,events:SEED_EVENTS,npcs:SEED_NPCS,qte:SEED_QTE,epMax:3000,rank:"NEW STANDER"},
    sprout:{zones:SPROUT_ZONES,story:SPROUT_STORY,events:SPROUT_EVENTS,npcs:SPROUT_NPCS,qte:SPROUT_QTE,epMax:10000,rank:"SEED ATIVO"},
    flower:{zones:FLOWER_ZONES,story:FLOWER_STORY,events:FLOWER_EVENTS,npcs:FLOWER_NPCS,qte:FLOWER_QTE,epMax:25000,rank:"SPROUT AVANCADO"},
  };
  function applyLanguageData(){
    const p=localePack();
    HUB_DATA=mergeData(BASE_HUB_DATA,p.hub);
    NPCS=mergeData(BASE_NPCS,p.npcs);
  }
  function getFlowData(flow){
    const base=BASE_FLOW_DATA[flow]||BASE_FLOW_DATA.seed,p=localePack(),ov=p.flows&&p.flows[flow]?p.flows[flow]:{};
    return {zones:mergeData(base.zones,ov.zones),story:mergeData(base.story,ov.story),events:cloneData(base.events),npcs:cloneData(base.npcs),qte:mergeData(base.qte,ov.qte),epMax:base.epMax,rank:rankTx(base.rank)};
  }
  
  function flowMeta(flow){return {seed:{c:"#00e832",i:"&#127793;",n:"SEED"},sprout:{c:"#00aaff",i:"&#127807;",n:"SPROUT"},flower:{c:"#9945ff",i:"&#127804;",n:"FLOWER"}}[flow];}
  function refreshFlowIndicator(){
    const cfg={seed:{c:"#00e832",i:"&#127793;",n:"SEED"},sprout:{c:"#00aaff",i:"&#127807;",n:"SPROUT"},flower:{c:"#9945ff",i:"&#127804;",n:"FLOWER"}};
    const d=cfg[currentFlow],el=document.getElementById("flowIndicator");
    if(!d||!el)return;
    el.innerHTML=ui().title.flowIndicator+": "+d.n+" "+d.i;el.style.color=d.c;
  }
  function selectFlow(flow){
    const d=flowMeta(flow);if(!d)return;
    SFX.init();SFX.play("select");haptic(30);currentFlow=flow;refreshFlowIndicator();
  }
  function loadFlow(flow){
    currentFlow=flow;ep=0;zi=0;wx=0;camX=0;trg={};
    const data=getFlowData(flow);
    ZONES=data.zones;STORY=data.story;WEVENTS=data.events;ZONE_NPCS=data.npcs;QTE_CFG=data.qte;epMax=data.epMax;rnk=data.rank;
    document.getElementById("hep").textContent="EP: 0";document.getElementById("efl").style.width="0%";
    const maxEl=document.querySelector("#hud .etr + .hep");if(maxEl)maxEl.textContent=epMax.toLocaleString();
    document.getElementById("hrnk").textContent=rnk;document.getElementById("hzone").textContent=ZONES[0].name;
  }
  function setupLanguageSelector(){
    const title=document.getElementById("titl"),indicator=document.getElementById("flowIndicator");if(!title||!indicator)return;
    if(indicator.previousElementSibling&&!document.getElementById("flowBtns"))indicator.previousElementSibling.id="flowBtns";
    if(indicator.previousElementSibling&&indicator.previousElementSibling.previousElementSibling&&!document.getElementById("flowLabel"))indicator.previousElementSibling.previousElementSibling.id="flowLabel";
    let sel=document.getElementById("langSel");
    if(!sel){sel=document.createElement("div");sel.id="langSel";sel.style.cssText="display:flex;gap:6px;flex-wrap:wrap;justify-content:center;align-items:center;margin:6px 0 8px";title.insertBefore(sel,document.getElementById("flowLabel")||indicator);}
    const names={pt:"PT",en:"EN",es:"ES"};
    sel.innerHTML=`<div style="font-family:'Share Tech Mono',monospace;font-size:9px;color:var(--dim);letter-spacing:2px;width:100%;text-align:center">${ui().title.language}</div>`+LANGS.map(l=>`<button class="hub-tbtn" style="padding:7px 18px;font-size:9px;border-color:${l===currentLang?'var(--green)':'rgba(255,255,255,.12)'};color:${l===currentLang?'var(--green)':'var(--dim2)'}" onclick="setLanguage('${l}')">${names[l]}</button>`).join("");
  }
  function updateHubCards(){
    const h=ui().hub;setText("#hub .hub-badge",h.badge);setText("#hub .hub-title",h.title);setText("#hub .hub-sub",h.sub);setText("#hub .hub-back-home",h.back);
    const cards=document.querySelectorAll("#hub .hub-card"),order=["seed","sprout","flower"];
    cards.forEach((card,i)=>{const k=order[i],c=h.cards[k],d=HUB_DATA[k];if(!c||!d)return;setText(card.querySelector(".hc-level"),c.level);const body=card.querySelector(".hc-body");if(body)body.innerHTML=`${c.body}<div class="hc-ep ${k}-ep">${d.epRange}</div>`;setText(card.querySelector(".hc-enter"),h.explore);});
  }
  function updateStaticText(){
    const t=ui();setupLanguageSelector();
    setText("#titl .tbdg",t.title.badge);setText("#titl .trpg",t.title.subtitle);setText("#titl .tsub",t.title.meta);setText("#tbtn",t.title.start);setHTML("#hubBtn",t.title.hub);setText("#flowLabel",t.title.chooseFlow);setText("#titl .thnt",t.title.tap);setText("#titl .tctrl",t.title.controls);refreshFlowIndicator();
    const fb=document.querySelectorAll("#flowBtns button"),fk=["seed","sprout","flower"];fb.forEach((b,i)=>{b.innerHTML=t.title.flowButtons[fk[i]]||b.innerHTML;});
    const reset=[...document.querySelectorAll("#titl button")].find(b=>(b.getAttribute("onclick")||"").includes("clearSave"));setText(reset,t.title.reset);
    setHTML("#dskip",t.dialog.skip);setText("#dtap",t.dialog.tap);setHTML("#qbok",t.qte.continue);
    const pause=document.getElementById("pauseOvl");if(pause){setText(pause.children[0],t.pause.title);setText(pause.querySelector(".sLabel"),t.pause.volume);const btns=pause.querySelectorAll("button");setHTML(btns[0],t.pause.continue);setText(btns[1],t.pause.restart);}
    updateHubCards();
  }
  function setLanguage(lang){
    if(!LANGS.includes(lang))return;
    currentLang=lang;try{localStorage.setItem("standx_lang",lang);}catch(e){}
    applyLanguageData();updateStaticText();
    if(gs==="title"&&document.getElementById("hubDetail").classList.contains("on")&&activeHubDetailRank)openHubDetail(activeHubDetailRank);
  }
  
  // ========== STATE ==========
  let ep=0,rnk="NEW STANDER",zi=0,wx=0,gs="title";
  let trg={},fc=0,bps=[];
  let currentFlow="seed",epMax=3000,activeHubDetailRank=null;
  ...
  function showCho(title,opts,cb){
    ...
    opts.forEach(o=>{
      ...
      const tag=o.tag||"";
      const tc=tag.includes("EP")||["DEEP","GENUINO","GENUINE","PROFUNDO"].includes(tag)?"tep":["LEARN","APRENDER"].includes(tag)?"tlrn":"trsk";
  ...
  function launchQTE(type){
    ...
    if(!cfg){gs="playing";ntf(ui().qte.unavailable,"warn");return;}
    gs="qte";
    const qel=document.getElementById("qte");qel.style.display="flex";
    document.getElementById("qres").style.display="none";
    const qcon=document.getElementById("qcon");qcon.style.display="";qcon.innerHTML="";
    ...
    if(cfg.opts)launchContent(cfg);else launchBar(cfg);
  }
  
  function launchBar(cfg){
    const qcon=document.getElementById("qcon");
    const q=ui().qte;
    qcon.innerHTML=`<div class="qbw"><div class="qtrk"><div class="qzo" style="left:${cfg.ok.from}%;width:${cfg.ok.to-cfg.ok.from}%"></div><div class="qzp" style="left:${cfg.pf.from}%;width:${cfg.pf.to-cfg.pf.from}%"></div><div class="qndl" id="qndl" style="left:0%"></div></div><div class="qlbs"><span class="qlb">${q.early}</span><span class="qlb o">${q.ok}</span><span class="qlb p">${q.perfect}</span><span class="qlb o">${q.ok}</span><span class="qlb">${q.late}</span></div></div><div class="qkh">${q.spaceTap}</div><div class="qtap" id="qtap" onclick="barPress()">${cfg.action||"AGIR"}</div>`;
    let pos=0,dir=1,last=performance.now();
    const state={done:false,pos:()=>pos,cfg,sh:null};
    function animN(now){if(state.done)return;const dt=(now-last)/16;last=now;pos+=dir*1.6*dt;if(pos>=100){pos=100;dir=-1;}if(pos<=0){pos=0;dir=1;}const n=document.getElementById("qndl");if(n)n.style.left=pos+"%";requestAnimationFrame(animN);}
  ...
  function launchContent(cfg){
    ...
    cfg.opts.forEach((o,i)=>{
      ...
      html+=`<div class="ccard" onclick="cPick(${i})"><div class="clbl">${o.lbl}</div><div class="ctxt">${o.txt}</div><div class="cdtg ${tc}" style="display:none" id="ctg${i}">${o.q==="deep"?q.deep:q.hype}</div></div>`;
    });
  ...
  function showQRes(icon,cls,title,lesson,pts,cb){
    ...
    document.getElementById("qrpts").textContent=pts+" "+ui().qte.pointsGained;
    document.getElementById("qbok").innerHTML=ui().qte.continue;
  ...
  function showZT(idx){
    const z=ZONES[idx];
    document.getElementById("zep").textContent=ui().zone+" "+(idx+1);
  ...
  function ntf(msg,type){
    const n=document.createElement("div");n.className="ntf "+type;n.textContent=tx(msg);
  ...
  function showEnd(){
    ...
    const end=ui().end,endRank=end.ranks[currentFlow]||"@SEED",featured=ep>=epMax*.8?`<br><span style="color:#ffe600">${end.featured}</span>`:"";
    el.innerHTML+=`<style>@keyframes pfx{0%{transform:translateY(0);opacity:1}100%{transform:translateY(80vh);opacity:0}}@keyframes flt{from{transform:translateY(0)}to{transform:translateY(-10px)}}</style>
      <div style="font-family:'Share Tech Mono',monospace;font-size:9px;color:rgba(0,232,50,.5);letter-spacing:4px">${end.episode}</div>
      ...
      <div style="font-family:Orbitron,monospace;font-size:clamp(18px,4vw,34px);font-weight:900;color:#00e832;text-shadow:0 0 30px rgba(0,232,50,.6);letter-spacing:4px">${end.evolution}</div>
      <div style="font-family:Orbitron,monospace;font-size:clamp(22px,5vw,42px);font-weight:900;color:#ffe600;text-shadow:0 0 20px rgba(255,230,0,.5);letter-spacing:6px">${endRank}</div>
      <div style="font-family:'Share Tech Mono',monospace;font-size:11px;color:#3a5060;line-height:2;max-width:380px">${end.points}: <span style="color:#00e832">${ep}</span>${featured}<br>${end.characters}</div>
      <div style="display:flex;gap:12px;flex-wrap:wrap;justify-content:center;margin:4px 0">${castHtml}</div>
      <div style="font-family:'Share Tech Mono',monospace;font-size:9px;color:rgba(255,230,0,.5);letter-spacing:3px;margin-top:4px">${end.next}</div>
      <button onclick="location.reload()" style="font-family:Orbitron,monospace;font-size:11px;letter-spacing:2px;padding:12px 38px;background:linear-gradient(135deg,#00e832,#00b020);color:#04080f;border:none;cursor:pointer;border-radius:4px;margin-top:8px">${end.playAgain}</button>
  ...
  resize();applyLanguageData();setupTitle();
